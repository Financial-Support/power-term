...
const defaults: Settings = {
  ...
  terminal_theme: 'default', quick_theme_panel_open: false, accent_dock_open: true,
  ai_endpoint: 'https://api.anthropic.com/v1/messages', ai_model: 'claude-sonnet-4-6',
  ai_include_terminal_context: false, updated_at: 0,
};
...
describe('SettingsModal', () => {
  ...
  it('renders Tags / Sync / AI / Updates tabs alongside Appearance and Terminal', () => {
    render(<SettingsModal onClose={() => {}} />);
    const tabs = screen.getAllByRole('tab');
    expect(tabs).toHaveLength(6);
    expect(tabs[2]).toHaveTextContent('Tags');
    expect(tabs[3]).toHaveTextContent('Sync');
    expect(tabs[4]).toHaveTextContent('AI');
    expect(tabs[5]).toHaveTextContent('Updates');
  });
});
